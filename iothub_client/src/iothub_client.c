        ...
        else
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    iot_queued_callback->iotHubClientHandle = iotHubClientInstance;
                    iot_queued_callback->userContextCallback = userContextCallback;
                    result = IoTHubClient_LL_SetDeviceTwinCallback(iotHubClientInstance->IoTHubClientLLHandle, iothub_ll_device_twin_callback, iot_queued_callback);
                    if (result != IOTHUB_CLIENT_OK)
                    {
                        LogError("IoTHubClient_LL_SetDeviceTwinCallback failed");
                        free(iot_queued_callback);
                    }
                }
            }
            (void)Unlock(iotHubClientInstance->LockHandle);
